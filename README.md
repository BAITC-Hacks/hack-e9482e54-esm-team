...
| `test_data/` | Небольшой дополнительный комплект CSV для ручной проверки загрузки |
...
### Проверка агента без платных вызовов

После установки зависимостей выполнить команды из предыдущего раздела с
`AGENT_LLM_MODE=replay`.

Проверить, что:

1. `python local_eval.py` завершился и вывел результат без падения агента
   и сообщений об отброшенных кампаниях.
2. Число проведённых пилотов больше нуля, расходы и контакты находятся в лимитах.
3. `python make_submission.py` создал `submission.csv` с 1–10 итоговыми кампаниями.

Поле «Кампаний» в отчёте локальной оценки включает пилоты. Количество именно
финальных кампаний выводит `make_submission.py`.

При проверке текущей версии на основном комплекте данных, seed 42 и
`AGENT_LLM_MODE=replay` получены: статус `PASS`, 19 пилотов, одна финальная
кампания, 4 167 контактов и расходы 38 754 у.е. Это воспроизводимый пример
работы локального симулятора, а не гарантия результата на скрытой среде.
...
5. Выбрать **«Загрузить CSV»** и добавить вместе три файла из `test_data/`:
   `customer_profile.csv`, `change_tariff.csv`, `dict_tariff.csv`.
...
Дополнительный комплект `test_data/` содержит 1 200 профилей, 1 080 переходов
и 4 тарифа. Это отдельные синтетические данные для демонстрации загрузки.
...
Публичная deployed-ссылка и конфигурация удалённого развёртывания в репозитории
не указаны. Доступен локальный запуск по адресу
[http://127.0.0.1:8501](http://127.0.0.1:8501) после выполнения команды Streamlit.
